...
const Bets = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selected) {
      try {
        ...
        alert("Error while placing the bet: " + error);
      } finally {
        setSelected(-1);
      }
    }
  ...
  return (
    <section>
      <h2 className="text-4xl text-white font-bold text-center mt-20 mb-10">
        ...
        <form onSubmit={handleSubmit}>
          <div className="w-full flex justify-center gap-x-7 items-center">
            <BetInput
              ...
              label={
                selected !== -1
                  ? Teams[findMatchSelected()?.team1 as string]?.name
                  : "Team 1"
              }
              disable={selected === -1}
            />
            ...
            <BetInput
              ...
              label={
                selected !== -1
                  ? Teams[findMatchSelected()?.team2 as string]?.name
                  : "Team 2"
              }
              disable={selected === -1}
            />
          ...
          <div className="w-full flex justify-center mt-6">
            <button
              disabled={selected === -1}
              className="relative px-12 py-3 rounded-xl bg-[#7747e5] text-white font-semibold transition-all duration-300 hover:bg-[#6a40d1]"
            >
              Let's do it
            </button>
          </div>
